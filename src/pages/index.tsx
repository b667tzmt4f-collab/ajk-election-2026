...
// ── Live results panel — single source of truth ─────────────────────────────
// This panel now reads from the SAME useLiveResults() hook that powers
// /live. Both pages query Supabase's `candidates` + `constituencies` tables
// directly, so there is exactly one source of vote data across the whole
// site. Whatever the data-entry sync (see /enter) writes to `votes_2026`
// appears here and on /live simultaneously — no separate dummy array to
...
export default function Home() {
  ...
  // Single source of truth: same hook /live uses, reading Supabase's
  // `candidates` + `constituencies` tables (which the sheet-sync writes to).
  const { seatResults, partyTally, loading: liveLoading } = useLiveResults()

  // Seats sorted numerically (LA-1, LA-2, ... LA-45 — never string-sorted,
  // which would order LA-1, LA-10, LA-11 ahead of LA-2).
    ...
    <div className={`dirA${theme === 'dark' ? ' dark' : ''}`}>
      ...
      <section className="a-hero">
        <div className="a-wrap a-herogrid">
          ...
          {/* Live results panel — single source: Supabase candidates/constituencies,
              same data /live reads. Updates in real time via the hook's
              postgres_changes subscription — no separate dummy array. */}
          <div className="a-livepanel">
            <div className="a-livepanel-h">
              ...
              <span className="a-livepanel-tag">
                {anyReporting ? 'LIVE' : liveLoading ? 'LOADING' : 'AWAITING RESULTS'}
              </span>
            </div>
      ...
      <style jsx global>{`
        ...
        .a-cd{display:flex;flex-wrap:wrap;align-items:center;gap:18px;margin-top:46px;
          padding:22px 26px;width:100%;max-width:100%;overflow:hidden;
          background:var(--card);border:1px solid var(--line);border-radius:14px;
          box-shadow:0 1px 2px rgba(12,19,32,.04);}
        .a-cd-lab{font-size:12px;letter-spacing:.06em;text-transform:uppercase;color:var(--muted);
          line-height:1.4;max-width:14ch;flex-shrink:0;}
        ...
        .a-counts{display:flex;gap:14px;margin-left:auto;flex-wrap:nowrap;min-width:0;}
        .a-count{text-align:center;min-width:0;flex:0 1 auto;}
